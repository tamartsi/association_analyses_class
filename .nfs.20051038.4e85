...
\begin{center}
\large“Generalization” is the replication of an association between a genetic variant and a trait, discovered in one population, to another population.
\end{center}
\begin{itemize}
\item  Most genetic association studies were performed in populations of European Ancestry (EA)
\item  These are often detected in very large GWAS (e.g. 100,000 individuals)
\end{itemize}
...
\begin{itemize}
\item To know, whether associations that were discovered in one populations exists in another. 
  \begin{itemize}
   \item This may not always be true...
   \end{itemize}
\item To gain power by limiting the number of variants tested for associations to those already previously reported. 
\item Because we need to perform \textit{replication} analysis, but we do not have access to an independent study with the same type of population and/or the same trait. 
\end{itemize}
